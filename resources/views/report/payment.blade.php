...
<div class="container">
    <form method="get" action="{{ url('paymentreportexcel') }}">
    ...
    <form method="get" action="">
        ...
        <div class="col-md-2 col-sm-4 col-xs-4">    
            <select name="quarter" id="quarter" class="form-control hide-print" >
                ...
                @foreach($all_quarters as $key => $value)
                <option 
                    @if ($key == $quarter)
                            {{ 'selected' }}
                    @endif 
        ...
        <div class="col-md-2 col-sm-4 col-xs-4">    
            ...
            @endif 
            >
                <option value="" selected disabled>~ወርሒ~</option>
                @foreach($all_months as $key => $value)
                    <option
                        @if ($key+1 == $month && $quarter == 1)
                                {{ 'selected' }}
                        @endif 
                        value="{{ $key+1 }}">
                    {{ $value }}</option>
    ...
    <div class="row">
        <div class="col-sm-12">
            <h4 style="text-align: center;">{{$quarter != 1 ? 'ናይ' : ''}} {{ $quarter == 1 ? $all_months[$month-1] : $all_quarters[$quarter] }} {{ $year }} [ወርሓዊ] ክፍሊት ሪፖርት {{ $zobadatas[$zoneCode] }}</h4>
