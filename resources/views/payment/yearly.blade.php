...
<div class="box box-primary">
    ...
    @include('layouts.partials.filter_html', ['address' => 'yearly'])
        ...
        <div class="table-responsive text-center">
          <div class="pull-left">
            <form method="get" action="{{ URL('yearlylist')}}">
          ...
          <span class="glyphicon glyphicon-tick"></span> ክፍሊት መዝግብ
			...
			<tbody>
                         @foreach ($data as $myyearly)
                           @if(true/*$myyearly->hitsuy->occupation != "መምህር" && $myyearly->hitsuy->occupation != "ሲቪል ሰርቫንት"*/)
                          <tr>
                            <td><input style="display: inline;" type="checkbox" class="checkbox" name="check[]" value="{{{ $myyearly->hitsuyID }}}"
                              @if(!isset($yearlydata[$myyearly->occupation]))
                              {{ 'disabled="disabled" '}}
                              @endif
                              ></td>
