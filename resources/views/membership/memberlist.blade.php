...
@section('main-content')
    ...
    @include('layouts.partials.filter_html', ['address' => 'memberlist'])
                 @foreach ($data as $mydata)                                     
                            <?php $bezhi +=1; ?>
         @endforeach
     <div>                      
            <span style="bold"በዝሒ ኣባላት: {{ $bezhi }} &nbsp
        </div>         
	<form method="get" action="{{ url('memberlistexcel') }}">
					...
					<table class="table table-borderless" id="table2">
						...
						<tbody>
							...
							<tr>
								...
								<td>{{ $mydata->hitsuy->educationLevel }}</td>
								<td>{{ str_replace(',', '፣', $mydata->assignedAssoc) }}</td>
